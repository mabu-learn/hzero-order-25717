...
表结构及初始化数据在 <a target="_blank" href="https://code.choerodon.com.cn/hzero-hzero/hzero-resource/tree/0.10.0.RELEASE"><b>hzero-resource</b></a> 项目下。
...
* database-init.sh：执行初始化的脚本
    ...
    ![](/img/docs/installation-configuration/install/1563380297.jpg)

### database-init.sh

* 脚本内容如图

    ![](/img/docs/installation-configuration/install/1563380641.jpg)
...
![](/img/docs/installation-configuration/install/1550746345.jpg)
...
* 以 hzero-gateway 服务为例，首先创建一个空的工程，一般以产品或编码为前缀命名，如 demo-gateway。
    ...
    ![](/img/docs/installation-configuration/install/demo_1550742871.jpg)
...
* 修改 `pom.xml`，可以 hzero-parent 为父 pom，也可自行建一个所属产品或项目的顶级 parent 项目进行统一版本管理，接着引入依赖的服务。
    ...
    ![](/img/docs/installation-configuration/install/demo_1550742991.jpg)
...
* 从依赖包中复制出配置文件到 `resources/` 目录下，主要修改 application.yml 中的一些配置即可。
    ...
    ![](/img/docs/installation-configuration/install/demo_1550745832.jpg)

* 修改启动类，加入对应的 `@EnableHZeroXxx` 注解，会自动扫描依赖服务的包。

    ![](/img/docs/installation-configuration/install/demo_1550745930.jpg)
...
* 修改 run.sh，将`JAR`设置为服务名，`MPORT`设置为配置文件中 `management.port` 的端口。该脚本会拉取最新代码、打包、停止原服务、启动新服务。
    ...
    ![](/img/docs/installation-configuration/install/1546075203.jpg)

* 设置 run.sh 可执行：`# chmod +x run.sh`

* Jenkins 添加 SSH 服务器：在系统管理>系统配置下，用于连接部署的服务器

    ![](/img/docs/installation-configuration/install/1546077096.jpg)

* 创建项目

    ![](/img/docs/installation-configuration/install/1546077246.jpg)

* 基础配置

    ![](/img/docs/installation-configuration/install/1546077358.jpg)

* 增加构建配置

    ![](/img/docs/installation-configuration/install/1546077416.jpg)

* 构建并启动服务

    ![](/img/docs/installation-configuration/install/1546077925.jpg)
...
* hzero_platform 的数据初始化完成后，请检查 `fd_organization`、`hpfm_tenant`、`hpfm_group` 三张表，是否都有一条ID=0的数据，由于表自增，需手动将 HZERO 这个租户的ID设置为0。  
    ![](/img/docs/installation-configuration/install/1546432910.jpg)

* 服务都部署成功后，可以查看注册中心上的服务是否都注册成功  

    *注册中心地址：`http://dev.hzero.org:8000`*

    ![](/img/docs/installation-configuration/install/1546430614.jpg)
...
* 访问 swagger 页面，测试API是否可用  
    ...
    *Swagger地址：`http://dev.hzero.org:8080/swagger/swagger-ui.html`*
    ...
    查看服务列表：  
    ![](/img/docs/installation-configuration/install/1546430697.jpg)

    例如找平台服务的API进行测试，首先需要授权，默认会跳转到 oauth 登录页面进行登录授权。之后便可访问API测试。
    ![](/img/docs/installation-configuration/install/1546433265.jpg)  
    ![](/img/docs/installation-configuration/install/1546433435.jpg)
